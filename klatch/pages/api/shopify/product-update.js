...
import { getProductQL } from 'data/graphql/products'
import { convertQLNodeToArray } from 'data/utils'
...
export default async function send(req, res) {
  ...
  const {
    body: { status, id, title, handle, options, variants, images },
  } = req
  ...
  const productGallery = images.length > 0 ? 
      images.map((image)=> ({
        _key: image.id,
        url: image.src,
  ...
  //const dt = convertToBlock(req.body.body_html)
  ...
  const productFields = {
    ...
    description: blockTools.htmlToBlocks(
      req.body?.body_html || '', 
      defaultSchema.get('blogPost').fields.find(field => field.name === 'body').type, 
      {
    ...
    inStock: variants.some(
      (v) => {
        ...
        return v.inventory_policy === 'deny'
      }
    ),
    lowStock: false,
    ...
    galleryPhotos: [{
      ...
      _key: `shopify-gallery-${id}`,
      shopifyPhotos: productGallery
    }],
  }
  ...
  const productVariantFields = variants
    ...
    .map((variant) => ({
      ...
      sku: variant.sku || '',
      apiId: variant.admin_graphql_api_id,
      inStock: variant.inventory_policy === 'deny' ||
      ...
      lowStock: variant.inventory_policy === 'deny'
        ? false
  ...
  const shopifyProduct = await axios({
    url: `https://${process.env.SHOPIFY_STORE_ID}.myshopify.com/admin/api/2022-01/graphql.json`,
    method: 'POST',
    data: getProductQL(id),
    headers: shopifyConfig,
  }).then(res => res.data.data.product)

  const metafields = convertQLNodeToArray(shopifyProduct?.metafields)
  // See if our metafield exists
  
  productCompare.productStandardType = productFields.productStandardType = shopifyProduct?.standardizedProductType?.productTaxonomyNode?.name || ''
  productCompare.collections = productFields.collections = convertQLNodeToArray(shopifyProduct?.collections)?.map(collection => {
    return collection.title
  })

  productCompare.seo = productFields.seo = {
    metaTitle: metafields.find(t => t.key === 'title_tag' )?.value || '',
    metaDesc: metafields.find(t => t.key === 'description_tag' )?.value || '',
    shareTitle: metafields.find(t => t.key === 'title_tag' )?.value || '',
    shareDesc: metafields.find(t => t.key === 'description_tag' )?.value || '',
  }
  productCompare.isGiftCard = productFields.isGiftCard = shopifyProduct?.isGiftCard || false;
  

  const previousSync = metafields?.find(
  ...
  //return res.json({finish: 'done'})} async function b(){
  // Metafield found
  if (previousSync) {
    ...
    if (jsondiffpatch.diff(JSON.parse(previousSync.value), productCompare)) {
      ...
      await axios({
        url: `https://${process.env.SHOPIFY_STORE_ID}.myshopify.com/admin/products/${id}/metafields/${previousSync.legacyResourceId}.json`,
        method: 'PUT',
        headers: shopifyConfig,
        data: {
          metafield: {
