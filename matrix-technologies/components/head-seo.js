...
const HeadSEO = ({ site = {}, page = {}, schema }) => {
  // set <head> variables
  const siteTitle = site.title || 'Default Site Title'
  ...
  const metaTitle = replaceTemplateTags(
    page.seo?.metaTitle || site.seo?.metaTitle || 'Default Meta Title',
    templateTags
  )
  const metaDesc =
    page.seo?.metaDesc || site.seo?.metaDesc || 'Default Meta Desc'

  const shareTitle = replaceTemplateTags(
    page.seo?.shareTitle || site.seo?.shareTitle || 'Default Share Title',
    templateTags
  )
  const shareDesc =
    page.seo?.shareDesc || site.seo?.shareDesc || 'Default Share Desc'
  const shareGraphic =
