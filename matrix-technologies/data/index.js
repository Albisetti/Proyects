...
import { homeID, shopID, errorID, product } from './utils'
import * as queries from './queries'

// Fetch all dynamic docs
export async function getAllDocSlugs(doc) {
  const data = await getSanityClient().fetch(
    `*[_type == "${doc}" && !(_id in [${homeID}, ${shopID}, ${errorID}]) && wasDeleted != true && isDraft != true]{ "slug": slug.current }`
  )
  return data
}

// Fetch all visible dynamic docs
export async function getAllVisibleDocSlugs(doc) {
  const data = await getSanityClient().fetch(
    `*[_type == "${doc}" && visibility == "visible" && wasDeleted != true && isDraft != true]{ "slug": slug.current }`
  )
  return data
}
