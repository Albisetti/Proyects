...
import { RESET_PASSWORD_REQUEST } from "../../lib/user";
...
const RESET_TOAST_FAILURE_TEXT = `Something went wrong with your password reset request. Please try again or contact ${APP_TITLE} for assistance. `;
const RESET_TOAST_SUCCESS_TEXT = `Password was updated successfully! Logged in.`;
...
function ResetPassword(props) {
    ...
    const [attemptPasswordUpdate] = useMutation(RESET_PASSWORD_REQUEST);
    ...
    async function handlePasswordResetAttempt() {
        ...
        try {
            ...
            const { token, user } = data.resetPasswordRequest;
            if (token === null || user === null) {
				toast.error(RESET_TOAST_FAILURE_TEXT);
