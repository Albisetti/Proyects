...
class MassSaveModels implements ArgResolver
{
    /**
    ...
    {
        //TODO: $model is only 1 that is a issue

        $models = collect([]);

//        throw new \Exception(json_encode($args));

        if( isset($args->arguments['organizations']) ) {
            foreach ( $args->arguments['organizations']->value as $organization ){
